...
// Global multiplier to increase overall amount of fireworks/stars.
// Increase this to make bursts denser and the show more intense.
const FIREWORK_AMOUNT_MULTIPLIER = 2.5;
...
function configDidUpdate() {
  ...
  if (skyLightingSelector() === SKY_LIGHT_NONE) {
    appNodes.canvasContainer.style.backgroundColor = radial-gradient('circle at top,#ffe3e3 0%, #f7bcbc 45%, #b02a2a 100%');
  }
...
let isFirstSeq = true;
// Increase finale count to create larger finales
...
function updateGlobals(timeStep, lag) {
  ...
  if (store.state.config.autoLaunch) {
    ...
    if (autoLaunchTime <= 0) {
      autoLaunchTime = startSequence() * 0.7;
    }
  }
}
...
class Shell {
  constructor(options) {
    ...
    if (!this.starCount) {
      const density = (options.starDensity || 1) * FIREWORK_AMOUNT_MULTIPLIER;
      const scaledSize = this.spreadSize / 54;
      // Multiply base star count so the overall show is denser when multiplier > 1
